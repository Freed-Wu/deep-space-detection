...
int main(int argc, char* argv[]) {
	...
	std::fstream file_bin("tmp.bin", std::ios::out | std::ios::binary);
	...
	//float symlow = prob1 * (normal_cdf(index - 0.5, mean1, std1)) + prob2 * normal_cdf(index - 0.5, mean2, std2) + prob3 * normal_cdf(index - 0.5, mean3, std3);
	//float symhigh = prob1 * (normal_cdf(index + 0.5, mean1, std1)) + prob2 * normal_cdf(index + 0.5, mean2, std2) + prob3 * normal_cdf(index + 0.5, mean3, std3);
	//int symlow_int = int(symlow * freqs_resolution);
	//int symhigh_int = int(symhigh * freqs_resolution);
	...
	for (int i = low_bound; i <= high_bound; i++) {
		...
		float highboundhigh =
			prob1 * (normal_cdf(i + 0.5, mean1, std1))
			+ prob2 * normal_cdf(i + 0.5, mean2, std2)
			+ prob3 * normal_cdf(i + 0.5, mean3, std3);
		if (i == index) {
			symlow = total_freqs;
		...
		total_freqs +=
			int((highboundhigh - lowboundlow) * freqs_resolution);
		if (i == index) {
			symhigh = total_freqs;
		}
		//std::cout << int((highboundhigh - lowboundlow) * freqs_resolution) << std::endl;
	}
